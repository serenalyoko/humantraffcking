...
def train_one_fold(X, y, train_idx, test_idx, device, args):
    ...
    train_loader = DataLoader(
        EmbDataset(X_tr, y_tr), batch_size=args.batch_size, shuffle=True
    )
    val_loader = DataLoader(
        EmbDataset(X_val, y_val), batch_size=1024, shuffle=False
    )
    test_loader = DataLoader(
        EmbDataset(X_te, y_te), batch_size=1024, shuffle=False
    )
    ...
    pos_weight = torch.tensor(
        [base_weight * args.pos_weight_factor],
        dtype=torch.float32,
        device=device,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_ap = (
            average_precision_score(y_val, val_probs) if val_probs.size else 0.0
        )
    ...
    if args.decision == "recall":
        thr = (
            choose_threshold_for_recall(y_val, val_probs, args.recall_target)
            if val_probs.size
            else 0.5
        )
    else:
        thr = (
            choose_threshold_by_f1(y_val, val_probs) if val_probs.size else 0.5
        )
    ...
    if args.verbose:
        print(
            f"[Fold] thr={metrics['threshold']:.3f} | "
            f"P={metrics['precision']:.3f} | R={metrics['recall']:.3f} | "
            f"F1={metrics['F1']:.3f} | ACC={metrics['accuracy']:.3f}"
        )

    y_pred = (
        (test_probs >= thr).astype(np.int64) if test_probs.size else np.zeros_like(y_te)
    )
    cm = confusion_matrix(y_te, y_pred, labels=[0, 1])
    fpr, tpr, _ = (
        roc_curve(y_te, test_probs) if test_probs.size else (np.array([0, 1]), np.array([0, 1]), None)
    )
...
def main():
    ...
    desc_col = df_labels["description"].astype(str) if "description" in df_labels.columns else None
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
        ...
        if args.plot:
            plot_confusion_matrix(
                cm, out_dir / f"cm_fold{fold}.png", f"Confusion Matrix - Fold {fold}"
            )
            fig = plt.figure()
    ...
    if args.plot and len(fold_details) > 0:
        ...
        plot_mean_roc(
            fprs, tprs, aucs, out_dir / "roc_mean.png", "Mean ROC across folds"
        )
    ...
    for i, m in enumerate(all_metrics, start=1):
        ...
        row.update(m)
        rows.append(row)
    pd.DataFrame(rows).to_csv(out_dir / "per_fold_metrics.csv", index=False)
    ...
    true_text = [id2label[int(t)] for t in oof_true]
    pred_text = [id2label[int(p)] for p in oof_pred]

    data = {
        "sample_idx": np.arange(n_samples),
        "true_label_id": oof_true,
        "true_label": true_text,
        "pred_prob_risk": oof_prob,
        "pred_label_id": oof_pred,
        "pred_label": pred_text,
    }
    if desc_col is not None:
        data["description"] = desc_col.values

    df_oof = pd.DataFrame(data)
    ...
    print(f"\nSaved per-sample OOF predictions to: {oof_path}")
